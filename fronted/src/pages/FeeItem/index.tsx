...
const FeeItemPage: React.FC = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try { const res = await getFeeItems({ page, pageSize, ...filters }); setData(res.data.records); setTotal(res.data.total); } catch { /* handled */ }
    setLoading(false);
  };
  ...
  const columns = [
    ...
    { title: '适用宿舍', dataIndex: 'applicableDormType' },
  ...
  return (
    <div style={{ padding: 24 }}>
      <div className="search-form">
        <Space wrap>
          <Select placeholder="收费类型" allowClear style={{ width: 150 }} onChange={(v) => setFilters((f) => ({ ...f, feeType: v || undefined }))}>
